...
void ThreadQueueMeshes::getQueue(std::queue<std::unique_ptr<MeshDataChunk>>& swapQueue)
{
...
void ThreadQueueMeshes::mergeQueue(std::queue<std::unique_ptr<MeshDataChunk>>& mergeQueue)
{
	std::lock_guard<std::mutex> lock(queueMutex);
